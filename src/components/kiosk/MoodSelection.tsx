  ...
  return (
    ...
    >
      ...
      <div className="w-full md:w-[35%] relative rounded-3xl overflow-hidden shadow-2xl border border-[#3E2723]/10 flex flex-col justify-end p-6 text-white min-h-[300px] md:min-h-0 group">
        ...
        <div className="relative z-10 space-y-4 text-right rtl:text-right ltr:text-left bg-black/30 backdrop-blur-lg border border-white/10 p-6 rounded-2xl shadow-2xl transition-all duration-300 group-hover:bg-black/40 group-hover:border-white/20">
          {/* Circular logo positioned top-right with integrated Instagram-style Story ring */}
          {cafe.logo && (
            <div className="absolute -top-3 -right-3 flex items-center justify-center">
              {/* Active indicator ring (Emerald green gradient ring) */}
              <div className="w-[60px] h-[60px] rounded-full p-[2.5px] bg-gradient-to-tr from-emerald-400 via-emerald-600 to-teal-400 shadow-md flex items-center justify-center">
                <div className="w-full h-full rounded-full bg-[#1F1210] p-[2px] flex items-center justify-center overflow-hidden">
                  <Image
                    src={cafe.logo ?? ''}
                    alt="Cafe Logo"
                    width={54}
          ...
          <div className="space-y-2 pr-14 ltr:pr-0 ltr:pl-14">
            ...
            <span className="text-[10px] text-amber-400 font-extrabold uppercase tracking-widest block drop-shadow-sm">
              {isAr ? 'مستشارك المزاجي للقهوة' : 'Your Personal Coffee Matcher'}
            </span>
            <p className="text-xs text-gray-200/90 leading-relaxed font-semibold drop-shadow-sm">
              {isAr
        ...
        <div className="space-y-4">
          ...
          <div>
            <motion.button
              ...
              className={`w-full py-5 rounded-2xl text-sm font-black shadow-xl flex items-center justify-center gap-2 transition-all cursor-pointer relative z-10 overflow-hidden ${selectedMood
                ? 'bg-gradient-to-r from-[#5D4037] via-[#3E2723] to-[#2D1C1A] hover:opacity-95 text-[#FAF8F5] ring-4 ring-[#3E2723]/15'
                : 'bg-[#3E2723]/35 text-[#FAF8F5]/50 cursor-not-allowed shadow-none'
                }`}
            >
              {selectedMood && (
