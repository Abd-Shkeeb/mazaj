...
export default async function CafeKioskPage({
  ...
  if (existingSessionId) {
    ...
    if (hasActiveSession) {
      ...
      console.log(`[KioskPage] ❌ Session invalid/used/expired. Showing scan-QR screen.`)
    }
  } else {
    console.log(`[KioskPage] No session cookie. Showing scan-QR screen.`)
  }

  // 4. If no active session → show scan-QR screen inline (no HTTP redirect = no 429 loop)
  if (!hasActiveSession) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-[#FAF8F5] px-4 py-12 relative overflow-hidden">
        <div className="absolute top-0 right-0 w-72 h-72 bg-[#3E2723]/5 rounded-full blur-3xl pointer-events-none select-none" />
