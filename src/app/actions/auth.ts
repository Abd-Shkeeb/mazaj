...
export async function loginAction(formData: { email: string; password?: string }) {
  try {
    ...
    if (!user) {
      return { success: false, error: 'User not found / البريد الإلكتروني غير مسجل' }
    }
    ...
    if (!user.cafeId && !isSuperAdmin) {
      return { success: false, error: 'User is not associated with any cafe / هذا الحساب غير مرتبط بمقهى' }
    }
    ...
    return { success: false, error: errMsg || 'Error occurred / حدث خطأ أثناء تسجيل الدخول' }
  }
}
